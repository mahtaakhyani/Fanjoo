...
var prompts = [
{
	prompt: 'ساخت کابینت آشپزخانه',
	weight: 1,
	class: 'group0'
},
{
	prompt: 'محافظت از پول در یک ماشین حفاظتی',
	weight: 1,
	class: 'group1'
},
{
	prompt: 'کار کردن در یک مزرعه‌ی لبنیاتی',
	weight: 1,
	class: 'group2'
},
{
	prompt: 'قرار دادن آجر یا کاشی',
	weight: 1,
	class: 'group3'
},
{
	prompt: 'نظارت بر یک ماشین در خط تولید',
	weight: 1,
	class: 'group4'
},
{
	prompt: 'تعمیر لوازم منزل',
	weight: 1,
	class: 'group5'
},
{
	prompt: 'رانندگی تاکسی',
	weight: 1,
	class: 'group6'
},
{
...
{
	prompt: 'ساخت یک مسیر عابر پیاده‌ی آجری',
	weight: 1,
	class: 'group9'
},
{
	prompt: 'نصب و وصل قطعات الکترونیکی',
	weight: 1,
	class: 'group10'
},
{
	prompt: 'رانندگی یک کامیون برای رساندن بسته‌های پستی به منازل و دفاتر',
	weight: 1,
	class: 'group11'
}
...
function createPromptItems() {
	...
	for (var i = 0; i < prompts.length; i++) {
		...
		// prompt_li.setAttribute('id', prompts[i].class);
		prompt_p.setAttribute('class', 'question fs-23 p-b-15');
...
function createValueButtons() {
	for (var li_index = 0; li_index < prompts.length; li_index++) {
		...
		for (var i = 0; i < prompt_values.length; i++) {
			...
			btn_group.className = 'option';

			var button = document.createElement('button');
			var button_text = document.createTextNode(prompt_values[i].value);
			// button_text.setAttribute('class', 'option_text');
	...
	// TweenLite.to(window, 2, {scrollTo: '#group1' });
	// }
	}
}
...
//scroll to the next question when option's selected by user.
var curr_el_index = 0;
var els_length = $(".list-group-item").length;

$('.value-btn').click(function () {
    curr_el_index++;
    if (curr_el_index >= els_length) curr_el_index = 0;
    $("html, body").animate({
        scrollTop: $(".list-group-item").eq(curr_el_index-1).offset().top - 20
    }, 500);
});

// $('.value-btn').on('change', function() {
//     var nextQuestion = $(this).closest('.list-group-item prompt').next();
...
/*==================================================================
    [ Validate ]*/
    var inputs = $('.validate-inputs .input100');
    var inputf = $('.validate-inputf .input100');
    ...
    $('.validate-formf').on('submit',function(){
        var check = true;

        for(var i=0; i<inputf.length; i++) {
            if(validate(inputf[i]) == false){
                showValidate(inputf[i]);
                check=false;
            }
        }
    ...
    $('.validate-formf .input100').each(function(){
        $(this).focus(function(){
           hideValidate(this);
        });
    });
